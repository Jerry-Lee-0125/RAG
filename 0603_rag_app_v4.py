...
- 1.新增功能：檔案摘要預覽
    在「檔案管理」對話框中新增「檔案摘要預覽」分頁，使用者可以點選目前資料庫內的任一檔案，AI 助手會即時生成該檔案內容的精簡摘要，幫助使用者快速了解文件重點，提升使用體驗。

- 2.移除功能：LLM 溫度設定控制
    為了確保生成內容的專業性與一致性，將 LLM 的溫度設定固定為 0.0，移除使用者調整溫度的選項，避免因為誤觸導致回答出現不必要的隨機性或不準確的資訊。

- 3.新增防呆機制:檢索結果為空時直接阻斷 LLM 生成
    在 RAG 檢索階段加入防呆機制，如果檢索到的相關文本為空，系統會直接阻斷後續的 LLM 生成過程，避免 LLM 在沒有足夠上下文的情況下產生不準確或無意義的回答，提升整體的使用體驗與可靠性。

- 4.新增版面設計：免責聲明固定於頁面底部
...
def get_llm(model_name, keep_alive_time=None):
    ...
    # 如果有特別指定 keep_alive 時間 (例如 "0s")，就把它加進參數裡
