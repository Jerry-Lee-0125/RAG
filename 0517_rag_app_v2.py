...
def build_vector_db(vectordb, ui_placeholder=None):
    ...
    update_ui("✅ **[階段 4/4]** 正在呼叫嵌入模型轉換向量並寫入資料庫...")
        ...
        for root, _, files in os.walk(processed_data_path):
            for file in files:
                if not file.startswith('.') and not file.startswith('~$'):
                    ...
                    file_data.append({"選取刪除": False, "檔案路徑": rel_path, "檔案大小 (MB)": round(size_mb, 3), "修改時間": mtime_str})
...
with st.sidebar:
    st.subheader("📁 資料庫管理")
    if st.button("開啟檔案管理", icon="⚙️", use_container_width=True):
...
if prompt_input := st.chat_input("請輸入關於病理科流程的問題..."):
    ...
    with st.chat_message("assistant"):
        ...
        # 取得最近幾筆對話紀錄作為上下文參考
        history_messages = st.session_state.messages[-7:-1]
        chat_history_str = ""
            ...
            with st.expander("查看檢索來源片段"):
                if docs:
                    for i, doc in enumerate(docs):
                        ...
                        st.caption(doc.page_content[:200] + "...") # 只顯示前 200 字避免版面過於冗長
                else:
